...
func maxValue(events [][]int, k int) int {
	...
	// DP stores delta from other cells.
	...
	for i := 0; i < n; i += 1 {
		for j := k - 1; j >= 0; j -= 1 {
			...
			// DP is only delta if we skip current event, we skip event
			// weight, not the delta.
			...
			doPush(i+1, j, dp[i][j]) // We do not stop the path, we exclude current contentValue!
		}
	}
	return ans
